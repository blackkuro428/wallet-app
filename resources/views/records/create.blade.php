<x-app-layout>
    <div class="max-w-7xl mx-[5px] sm:mx-auto px-6 lg:px-8 py-12">
        <div class="mb-6">
            <a href="{{route('records.index')}}" class="hover:text-gray-900 text-xl flex items-center gap-1">
                &lt; 戻る
            </a>
        </div>
        <form action="{{route('records.store')}}" method="POST" class="max-w-md mx-auto">
            @csrf

            <!-- フォーム共通部品呼び出し -->
            @include('records.form-field', ['categories'=>$categories])
